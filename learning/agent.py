...
class QLearner():
  ...
  def __init__(self, policy, representation, discount_rate =0.8, learning_rate = 0.05, seed = 1):
    ...
    weight_dimension = 9
    self.weights = numpy.array([self.random_state.rand()-0.5 for i in range(weight_dimension)], dtype = float)
  ...
  def learn(state, action, reward, next_state, possible_actions):
    # w <-- w + learning_rate * (reward + lambda_ * max[Q(s', a')] - Q(s,a)) * (gradient of Q(s,a) wrt weights)
    ...
    new_weights = self.weights + (learning_rate * (reward + lambda_*max_Q_prime_a_prime - Q_s_a)) * state_action_feature_vector
